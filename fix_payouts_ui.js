...
const file = 'app/staff/admin/(dashboard)/payouts/page.tsx';
let content = fs.readFileSync(file, 'utf8');

content = content.replace(
  /<div className="p-8 max-w-7xl mx-auto space-y-8">[\s\S]+/m,
              ...
              {isLoading ? (
                ...
                payouts.map((record) => (
                  <tr key={record.deliveryPersonId} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-center">
                      <button className="text-xs font-medium text-gray-900 bg-white border border-[#EFEFEF] px-4 py-2 rounded-xl hover:bg-gray-50 transition-colors shadow-sm opacity-0 group-hover:opacity-100 focus:opacity-100">
                        Process
                      </button>
                    </td>
                  </tr>
