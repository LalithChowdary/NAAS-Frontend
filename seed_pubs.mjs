...
async function main() {
    ...
    console.log("Logged in successfully. Injecting publications...");

    let successCount = 0;
    for (const pub of publications) {
        const pubRes = await fetch(`${BASE_URL}/admin/publications`, {
            method: "POST",
            headers: {
        ...
        if (pubRes.ok) {
            console.log(`✅ Successfully added '${pub.name}'`);
            successCount++;
        } else {
            console.error(`❌ Failed to add '${pub.name}':`, await pubRes.text());
        }
    }

    console.log(`\nDone! Successfully injected ${successCount}/${publications.length} publications.`);
}
